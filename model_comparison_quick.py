"""
Quick model comparison on a subset of data for speed.
"""
...
def run_model_comparison(ticker, start_date="2020-01-01", end_date=None):
    print(f"\n{'='*60}")
    print(f"Model comparison for {ticker} from {start_date} to {end_date or 'present'}")
    print('='*60)
    
    # Load and prepare data
    raw_df = load_asset(ticker, start_date, end_date)
    ...
    # 1. Gaussian HMM with different K and covariance types (limited)
    k_values = [2, 3, 4]
    cov_types = ['full', 'diag']  # just two for speed
    for k in k_values:
...
if __name__ == "__main__":
    # Run on a few assets: the winning cases and a couple more, but with recent data for speed
    tickers = ["SPY", "NVDA", "DIS", "ADBE", "QQQ"]
    all_results = {}
    for ticker in tickers:
        try:
            df = run_model_comparison(ticker, start_date="2020-01-01")
