...
\begin{itemize}
    \item \textbf{Naive - train mean:} זהו השם שנשמר ב־artifact. בהערכת ה־Test הסופית התחזית הקבועה מבוססת למעשה על ממוצע \(\texttt{next\_log\_return}\) בכל היסטוריית ה־pre-Test הזמינה, כלומר Train+Validation, לאחר סיום model selection.
    \item \textbf{Naive persistence:} מנבא שהכיוון הבא יהיה זהה לכיוון האחרון שנצפה.
    \item \textbf{Moving Average 5:} כלל סיבתי המבוסס על ממוצע נע קצר של תשואות זמינות עד היום הנוכחי.
    \item \textbf{Discrete Markov Chain:} מודל מעבר נצפה בין Up/Down ללא Hidden States; הפרמטרים נאמדים מהיסטוריית ה־pre-Test בלבד.
\end{itemize}
