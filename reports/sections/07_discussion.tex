...
בניגוד ל־classifier, HMM בונה ייצוג מפורש של מצב לטנטי ושל דינמיקת המעבר בין states. ב־SPY ההפרדה בין states ניכרת במספר ממדים בו־זמנית: return, volatility, range, drawdown ו־dwell time. ה־posterior confidence גבוה ברוב הימים, אך האנטרופיה עולה סביב switch. בנוסף, VIX שלא שימש באימון מפריד בין חלק מה־states, והקורלציה של SPY עם נכסים אחרים משתנה לפי state.

לכן ה־HMM מספק \textbf{conditional market context}. במקום לשאול רק ``האם מחר יהיה Up?'', ניתן לשאול ``באיזה סוג סביבה סטטיסטית אנחנו נמצאים, כמה המודל בטוח בכך, וכיצד נכסים אחרים נוטים להתנהג בסביבה זו?''. זו יכולת שאינה נמדדת ישירות ב־DPA.

\subsection{Expressiveness מול robustness}
...
\subsection{משך המשטר והנחת Markov}
...
בסיכום, התמונה העקבית ביותר היא שה־Gaussian HMM מתאים בפרויקט זה יותר ל־\textbf{latent state estimation} מאשר ל־\textbf{point forecasting}. זו אינה נסיגה מהמטרה המקורית אלא ממצא אמפירי שמגדיר בצורה מדויקת יותר היכן המודל מספק ערך.
